...
@interface JLBleAssistManager() <CBCentralManagerDelegate, CBPeripheralDelegate>
...
@implementation JLBleAssistManager

+ (instancetype)sharedInstance {
    static JLBleAssistManager *singleton = nil;
 ...
 *  逻辑同 JLBleManager.syncSystemConnectedPeripherals。
 */
        ...
        if (_bleManager.state == CBManagerStatePoweredOn) {
            ...
            dispatch_after(0.5, dispatch_get_main_queue(), ^{
                if (weakSelf.bleManager.state == CBManagerStatePoweredOn) {
                    ...
                    [weakSelf syncSystemConnectedPeripherals];
                    if (weakSelf.blePeripheralArr.count > 0) {
                        [[NSNotificationCenter defaultCenter] postNotificationName:kFLT_BLE_FOUND object:weakSelf.blePeripheralArr];
                    }
                }
            });
        }
...
- (void)otaFuncWithFilePath:(NSString *)otaFilePath {
    ...
    [JLBleAssistManager sharedInstance];
    ...
    [otaManager cmdOTAData:otaData Result:^(JL_OTAResult result, float progress) {
        for (id<JLBleAssistManagerOtaDelegate> objc in self.delegates) {
